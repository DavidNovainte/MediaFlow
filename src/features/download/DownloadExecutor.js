...
class DownloadExecutor {
    ...
    async startDownload(savedOptions = null) {
        ...
        // 🛡️ Daily Limit Check
        if (!await this.manager.checkDailyLimit()) return;
        ...
        try {
            ...
            if (result.success) {
                ...
                // 📈 Increment Daily Count (Free User Limit)
                await this.manager.incrementDailyCount();
            } else {
    ...
    async downloadPlaylist() {
        ...
        try {
            ...
            if (errors === 0) this.app.showToast(window.i18n?.t('download.playlistDone', { count: completed }) || `🎉 All ${completed} videos downloaded successfully!`, 'success');
