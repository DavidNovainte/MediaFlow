...
class HistoryEvents {
    ...
    async shareToMobile(filePath, title) {
        // QR / mobile bridge is Pro — Community has no mobile module
        const isCommunity = window.FeatureFlags?.EDITION === 'community';
        const isPro = !!this.app?.licenseManager?.checkFeatureAccess?.();
        if (isCommunity || !isPro) {
            this.app?.showToast?.(
                window.i18n?.t('history.qrProOnly') ||
                    window.i18n?.t('common.proFeatureMsg') ||
                    'Show QR / mobile share is a Pro feature.',
                'warning'
            );
            try {
        ...
        try {
            if (!window.mediaflow?.mobileflow?.getFileQR) {
                this.app.showToast(
                    window.i18n?.t('history.qrProOnly') ||
                        'Mobile share is unavailable in this edition.',
                    'warning'
                );
                this.app?.router?.switchPage?.('upgrade');
                return;
    ...
    showContextMenu(x, y, item) {
        ...
        const isCommunity = window.FeatureFlags?.EDITION === 'community';
        const isPro = !!this.app?.licenseManager?.checkFeatureAccess?.();
        const showQrAsPro = isCommunity || !isPro;
        ...
        const actions = [
            ...
            { icon: ICO.folder, text: window.i18n?.t('history.openFolder') || 'Open folder', action: () => this.openHistoryFolder(item) },
            {
                icon: ICO.qr,
                text: showQrAsPro
                    ? (window.i18n?.t('history.showQRCodePro') ||
                        `${window.i18n?.t('history.showQRCode') || 'Show QR'} · Pro`)
                    : (window.i18n?.t('history.showQRCode') || 'Show QR'),
                class: showQrAsPro ? 'is-pro-gated' : '',
