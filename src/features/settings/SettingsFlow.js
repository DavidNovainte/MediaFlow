...
class SettingsFlow {
    ...
    async sendTestReport() {
        ...
        try {
            ...
            const status = await window.mediaflow?.license?.getStatus?.().catch(() => ({ isPro: false })) || { isPro: false };
            const hwid = await window.mediaflow?.license?.getHWID?.().catch(() => 'unknown') || 'unknown';
            const reportData = {
                type: 'TEST_REPORT',
                version,
                hwid,
