...
class MediaFlowApp {
    constructor() {
        ...
        this.licenseManager = new window.LicenseManager(this);
    ...
    bindEvents() {
        ...
        document.getElementById('btn-open-extension-folder')?.addEventListener('click', () => {
            if (!this.licenseManager?.checkFeatureAccess()) {
                this.showToast(window.i18n?.t('common.extProFeature') || 'Browser extension requires an activated MediaFlow license', 'warning');
                this.switchPage('upgrade');
                return;
            }
