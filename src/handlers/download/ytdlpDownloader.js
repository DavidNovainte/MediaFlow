...
const { FREE_DAILY_DOWNLOAD_LIMIT } = featureFlags;
const { assertPro } = require('../../utils/requirePro');
const { createProgressThrottler } = require('../../utils/progressThrottle');
const isTestEnv = process.env.NODE_ENV === 'test';

/** Batch / playlist bulk capture is Pro; Community edition ships without batch UI/code. */
function isBatchDownloadRequest(options = {}) {
    if (!options || typeof options !== 'object') return false;
    if (options.batch === true || options.isBatch === true || options.isPlaylist === true) return true;
    if (options.source === 'batch' || options.source === 'playlist' || options.source === 'queue') return true;
    if (options.playlistItems && String(options.playlistItems).includes(',')) return true;
    return false;
}

function assertBatchAllowedForEdition() {
    if (featureFlags && featureFlags.EDITION === 'community') {
        return {
            success: false,
            error: 'COMMUNITY_NO_BATCH',
            message: 'Batch / queue download is not available in MediaFlow Community. Use Pro for bulk capture.'
        };
    }
    return null;
}
...
function isDailyDownloadLimitEnabled() {
    const limit = Number(FREE_DAILY_DOWNLOAD_LIMIT);
    return Number.isFinite(limit) && limit > 0;
}

async function assertFreeDownloadQuota() {
    // 0 / null / undefined => unlimited free single downloads
    if (!isDailyDownloadLimitEnabled()) return null;

    try {
        const status = await licenseManager.getStatus();
        if (status?.isPro) return null;
    } catch {
        // treat as free on status failure
    }
    const limit = Number(FREE_DAILY_DOWNLOAD_LIMIT);
    const today = new Date().toISOString().split('T')[0];
    const quota = store.get('downloadQuota') || { date: today, count: 0 };
    if (quota.date !== today) {
        quota.date = today;
        quota.count = 0;
        store.set('downloadQuota', quota);
    }
    if (quota.count >= limit) {
        return {
            success: false,
            error: 'DAILY_LIMIT',
            message: `Daily free download limit reached (${quota.count}/${limit})`,
            count: quota.count,
            limit
        };
    }
...
function incrementFreeDownloadCount() {
    // Keep optional telemetry counter even when limit is disabled
    try {
        const today = new Date().toISOString().split('T')[0];
        const quota = store.get('downloadQuota') || { date: today, count: 0 };
        if (quota.date !== today) {
...
function setupDownloadHandlers(ipcMain) {
    ipcMain.handle('video:download', async (event, options) => {
        try {
            const opts = options || {};
            if (isBatchDownloadRequest(opts)) {
                const communityDenied = assertBatchAllowedForEdition();
                if (communityDenied) return communityDenied;
                const proDenied = await assertPro();
                if (proDenied) return proDenied;
            }
            const denied = await assertFreeDownloadQuota();
            if (denied) return denied;
            const result = await downloadVideo({ ...opts, sender: event.sender });
            if (result && result.success) {
                try {
                    const status = await licenseManager.getStatus();
                    if (!status?.isPro) incrementFreeDownloadCount();
                } catch {
