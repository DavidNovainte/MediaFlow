...
class RendererLogger {
    ...
    async submitFullReport() {
        ...
        try {
            // 收集基础信息
            const status = window.mediaflow?.license ? await window.mediaflow.license.getStatus() : { isPro: false };
            const hwid = window.mediaflow?.license ? await window.mediaflow.license.getHWID() : 'unknown';
